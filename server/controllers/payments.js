...
exports.capturePayment = async (req, res) => {
  ...
  console.log("✅ Starting capturePayment...");
  ...
  for (const course_id of courses) {
    try {
      ...
      const uid = new mongoose.Types.ObjectId(userId);
      if (course.studentsEnroled.includes(uid)) {
        console.log("ℹ️ Student already enrolled:", userId);
        return res.status(400).json({ success: false, message: "Student already enrolled" });
      }

      total_amount += course.price;
    } catch (err) {
      console.error("❌ Error during course lookup:", err.message);
      return res.status(500).json({ success: false, message: "Course lookup failed" });
    }
  }
